...
def get_Binary_Data(dir, PM):
    ...
    if not PM:
        ...
        try: #Read in data, with error handling for file reading issues.
            h1 = MesaData(f"{dir}/binary_logs_1/history.data") #Primary or donor star
            h2 = MesaData(f"{dir}/binary_logs_2/history.data") #Secondary or companion star
...
#Function for checking if the inputting PM value is a boolean. 
def PM_check(PM):
...
#Function for checking if the user has latex installed or not.
def latex_check():
    ...
    try:
        if parameter in profiles_parameters:
            ...
            if which_star == '1' or which_star == '2':
                ...
                if profile_n_input == "One":
                    ...
                    which_profile = input()
                    profile = MesaData(f"{dir}/binary_logs_{which_star}/profile{which_profile}.data")
                    data = profile.data(parameter)
                elif profile_n_input == "All":
                    data = []
                    profiles_index = MesaProfileIndex(f"{dir}binary_logs_{which_star}/profiles.index")
                    profiles = profiles_index.profile_numbers
                    for i in profiles:
                        profile = MesaData(f"{dir}/binary_logs_{which_star}/profile{i}.data")
        ...
        if parameter in history_parameters:
            ...
            if history_input == '1' or history_input == '2':
                h = MesaData(f"{dir}/binary_logs_{history_input}/history.data")
...
def get_Time(dir, time):
    ...
    which_star = input()
    if which_star == '1' or which_star == '2':
        profiles_index = MesaProfileIndex(file_name = f"{dir}/binary_logs_{which_star}/profiles.index")
        profiles = profiles_index.profile_numbers
        ages = []
        for i in range(len(profiles)):
            ages.append(get_profile_age(dir, f"binary_logs_{which_star}", i))
        time = reverse_Label_Star_age(time)
        nearest_time = nearest(ages, time)
        index = ages.index(nearest_time) + 1
        labeled_time = Label_Star_Age(nearest_time)
        labeled_time = labeled_time.replace("Star Age: ", "")
        profile = MesaData(f"{dir}/binary_logs_{which_star}/profile{index}.data")
        return labeled_time, index, profile
    else:
...
def plot_Abundances(dir, fps, threshold, PM):
    ...
    try: #Read in data, with error handling for file reading issues
        M1 = mp.MESA()
        M1.loadHistory(f"{dir}/binary_logs_1")
        profiles_index_1 = MesaProfileIndex(file_name=f"{dir}/binary_logs_1/profiles.index")

        #For star ages
        m1 = MesaLogDir(log_path= f"{dir}/binary_logs_1")
        h1 = MesaData(file_name= f"{dir}/binary_logs_1/history.data")
    ...
    for j in profiles_1:
        profile = MesaData(f"{dir}/binary_logs_1/profile{j}.data")
    ...
    if not PM:
        ...
        try: #Read in data, with error handling for file reading issues
            M2 = mp.MESA()
            M2.loadHistory(f"{dir}/binary_logs_2")
            profiles_index_2 = MesaProfileIndex(file_name = f"{dir}/binary_logs_2/profiles.index")

            #Star 2 ages
            m2 = MesaLogDir(log_path = f"{dir}/binary_logs_2")
            h2 = MesaData(file_name = f"{dir}/binary_logs_2/history.data")
        except (OSError, IOError, FileNotFoundError) as e:
            print(f"Error: Could not read the data files. Check your paths.")
            print(f"Details: {e}")
            return None
        ...
        for k in profiles_2:
            profile = MesaData(f"{dir}/binary_logs_2/profile{k}.data")
...
def plot_Kippenhahn(dir, PM):
    ...
    if PM:
        ...
        gs = gridspec.GridSpec(1, 1, figure=fig)
        ax1 = plt.subplot(gs[0, 0])
        handles = plot_kip(dir, 'binary_logs_1', "Donor", ax1, PM)
    ...
    if not PM:
        ...
        handles = plot_kip(dir, 'binary_logs_1', "Donor", ax1, PM)
        plot_kip(dir, 'binary_logs_2', "Companion", ax2, PM)
    ...
    plt.savefig(f"{dir}_Kippenhahn.png", dpi=300)
    plt.show()
...
#Already have get_energy_gen function from the Kippenhahn section, just modified a bit here.

def get_EOS_energy_gen(dir, logs, index):
    ...
    if PM:
        try:
            ...
            for i in range(len(profiles_1)):
                ...
                energy_1 = get_EOS_energy_gen(dir, 'binary_logs_1', i) #Energy generation
                temp_1 = get_profile_temp(dir, 'binary_logs_1', i) #Temperature
                density_1 = get_profile_density(dir, 'binary_logs_1', i) #Density
                mass_1 = get_profile_mass(dir, 'binary_logs_1', i) #Mass
                age_1 = get_profile_age(dir, 'binary_logs_1', i) #Age
    ...
    if not PM:
        try:
            ...
            for i in range(n_frames):
                ...
                energy_1 = get_EOS_energy_gen(dir, 'binary_logs_1', i) #Energy generation
                energy_2 = get_EOS_energy_gen(dir, 'binary_logs_2', i)
                temp_1 = get_profile_temp(dir, 'binary_logs_1', i) #Temperature
                temp_2 = get_profile_temp(dir, 'binary_logs_2', i)
                density_1 = get_profile_density(dir, 'binary_logs_1', i) #Density
                density_2 = get_profile_density(dir, 'binary_logs_2', i)
                mass_1 = get_profile_mass(dir, 'binary_logs_1', i) #Mass
                mass_2 = get_profile_mass(dir, 'binary_logs_2', i)
                age_1 = get_profile_age(dir, 'binary_logs_1', i) #Age
                age_2 = get_profile_age(dir, 'binary_logs_2', i)
...
def plot_equation_of_state(temp, density, energy_gen, age, mass, which_star, ax):
    ...
    #Clear plot to avoid overlaying previous plot on current plot
    ax.clear()
    ...
    d1, t1 = mask_breaks(density, temp, energy_mask_1)
    d2, t2 = mask_breaks(density, temp, energy_mask_2)
    d3, t3 = mask_breaks(density, temp, energy_mask_3)
    ...
    ax.plot(d1, t1, linewidth=15, color="#EE7733", label=f"$>$ 1 erg $g^{-1}$ $s^{-1}$")
    ax.plot(d2, t2, linewidth=15, color="#CC3311", label=f"$>$ 1000 erg $g^{-1}$ $s^{-1}$")
    ax.plot(d3, t3, linewidth=15, color="#EE3377", label=f"$>$ 10$^7$ erg $g^{-1}$ $s^{-1}$")

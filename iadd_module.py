...
#All must be strings. logs1 is the path to the logs folder for star 1.
#binary_history is the path to the binary_history.data file.
def Get_Binary_Data(logs1, logs2, binary_history):
    
    if (type(logs1) != str or type(logs2) != str or type(binary_history) != str): #Check that all inputs are strings.
        print('logs1, logs2, and binary_history must all be given as strings')
        return None

    if (logs1 == logs2): #Check that logs1 and logs2 are different to avoid reading the same data twice.
        print('logs1 and logs2 must lead to different data directories')
        return None
    
    try: #Read in data, with error handling for file reading issues.
        h1 = MesaData(logs1 + '/history.data') #Primary or donor star
        h2 = MesaData(logs2 + '/history.data') #Secondary or companion star
        b = MesaData(binary_history) #Binary data
    ...
    #Interpolate Masses, Age, Period, and Separation. They have shorter lengths than the Radii, Luminoisites, and Temperatures due to how the binary data is recorded.
...
def Label_Star_Age(age_y, i):
    label = ''
    if age_y[i] >= 1e6 and age_y[i] < 1e9: #Millions of years
        label = 'Star Age: ' + str(round(age_y[i] / 1e6, 3)) + ' Myr'
    elif age_y[i] >= 1e9: #Billions of years
        label = 'Star Age: ' + str(round(age_y[i] / 1e9, 3)) + ' Gyr'
    else: #Years
        label = 'Star Age: ' + str(round(age_y[i], 3)) + ' yr'
    return label
...
def plot_Mass_Transfer(logs1, logs2, binary_history, fps):

    if not fps_check(fps):
        return

    data = Get_Binary_Data(logs1, logs2, binary_history)
    if data is None:
        return 

    Age, a, M1, M2, Total_M, R1, R2, T1, T2, Lum1, Lum2, period = data

    #Calculate colors for animation
    ...
    os.makedirs("Mass_Transfer_frames", exist_ok=True)
    frames = []
    ...
    for i in range(n_frames):
        ...
        Ages = Label_Star_Age(Age, i)
        ax1 = plt.subplot(gs[0:4, 0:5])
        ax1.set_facecolor('black')
        plot_MT(ax1, Age[:i+1], M1[:i+1], M2[:i+1], Total_M[:i+1], colors1[:i+1], colors2[:i+1], MT_xmin, MT_xmax, MT_ymax)

        filename = f"Mass_Transfer_frames/frame_{i:03d}.png"
    ...
    clip.write_videofile("Mass_Transfer.mp4", codec="libx264")

#Actual plotting function.
def plot_MT(ax, Age, M1, M2, Total_M, colors1, colors2, MT_xmin, MT_xmax, MT_ymax):
    fontsize = 19
    ...
    plt.rcParams['text.usetex'] = True #Latex formatting enabled.
...
#For examples, see documentation.
#fps is the frames/s of the generated movie, ~12 generally provides good temporal resolution. Must be a positive integer.
def plot_Herzsprung_Russel(logs1, logs2, binary_history, fps, observers):
    ...
    #Frame generation
    os.makedirs("Hertzsprung_Russel_frames", exist_ok=True)
    frames = []
    ...
    for i in range(n_frames):
        ...
        ax1.set_facecolor('black')
        plot_HR(ax1, T1[:i+1], Lum1[:i+1], T2[:i+1], Lum2[:i+1], colors1[:i+1], colors2[:i+1], observers)
        ...
        filename = f"Hertzsprung_Russel_frames/frame_{i:03d}.png"
        plt.savefig(filename, dpi=100)
        plt.close(fig)
        frames.append(filename)
    ...
    #Create animation
    clip = ImageSequenceClip(frames, fps=fps)
    clip.write_videofile("Hertzsprung_Russel.mp4", codec="libx264")

#Actual Plotting Function
def plot_HR(ax, T1, Lum1, T2, Lum2, colors1, colors2, observers):
...
plot_Herzsprung_Russel("LOGS1", "LOGS2", "binary_history.data", 12, observers=True)
...
#For examples, see documentation.
#fps is the frames/s of the generated movie, ~12 generally provides good temporal resolution. Must be a positive integer.
def plot_Roche_lobe(logs1, logs2, binary_history, fps):
    ...
    #Frame generation
    os.makedirs("Roche_Lobe_frames", exist_ok=True)
    frames = []
    ...
    for i in range(n_frames):
        ...
        ax1 = plt.subplot(gs[0:1, 0:1])
        ax1.set_facecolor('black')
        plot_RL(ax1, lpoints, lpot, p, R1[i], R2[i], a[i], Ages, ext_vect, q, colors1[i], colors2[i])
        
        filename = f"Roche_Lobe_frames/frame_{i:03d}.png"
        plt.savefig(filename, dpi=100)
        plt.close(fig)
        frames.append(filename)
    ...
    #Create animation
    clip = ImageSequenceClip(frames, fps=fps)
    clip.write_videofile("Roche_Lobe.mp4", codec="libx264")
    
    return

#Actual plotting function
def plot_RL(ax, lpoints, lpot, p, R1, R2, a, Ages, ext_vect, q, colors1, colors2):
    ...
    ax.text(0.015, 0.98, Ages, transform=ax.transAxes,
...
#Energy generation as a function of density

#For examples, see documentation
#fps is the frames/s of the generated movie, ~12 generally provides good temporal resolution.
#Must be a positive integer

#plot_Energy_Generation uses many of the functions defined in the Kippenhahn diagram section.
#To see how those functions work, go to that section. 

def get_density(logs, index):

    index += 1
    profile = MesaData(f"{logs}/profile{index}.data")

    rho = profile.data('')

def plot_Energy_Generation(logs1, logs2, binary_history, fps):
...
def plot_en_gen(logs, which_star, ax):

    ages, profiles (get_profile_and_ages(logs))

    ages = np.array(ages)
    Star_age = Label_Star_Age(ages)
    ...
    #X axis formatting
    ax.set_xlabel('Mass', color='white', labelpad=0, fontsize=fontsize)
    ax.tick_params(axis='x', colors='white', labelsize=fontsize)
...
def element_remover(abundances, elements, threshold):
    to_remove = []
...
#logs1 is the path to the logs folder for star 1, etc.
#binary_history is the path to the binary_history.data file.
#All must be strings.
...
def plot_Abundances(logs1, logs2, binary_history, fps, threshold):
    ...
    M1 = mp.MESA()
    M1.loadHistory(f= logs1)

    profiles_index_1 = MesaProfileIndex(file_name= logs1 + '/profiles.index')
    profiles_1 = profiles_index_1.profile_numbers

    #Mesa object and profiles for star 2
    M2 = mp.MESA()
    M2.loadHistory(f= logs2)

    profiles_index_2 = MesaProfileIndex(file_name= logs2 + '/profiles.index')
    profiles_2 = profiles_index_2.profile_numbers

    #List of elements in each star
    elements_1 = get_isotopes(M1)[:-1]
    elements_2 = get_isotopes(M2)[:-1]
    
    #Star 1 ages
    m1 = MesaLogDir(log_path= logs1)
    h1 = MesaData(file_name= logs1 + '/history.data')
    ...
    #Star 2 ages
    m2 = MesaLogDir(log_path= logs2)
    h2 = MesaData(file_name= logs2 + '/history.data')
    ...
    for j in profiles_1:
        profile = MesaData(f"{logs1}/profile{j}.data")
    ...
    for k in profiles_2:
        profile = MesaData(f"{logs2}/profile{k}.data")
    ...
    elements_1, abundances_data_1 = element_remover(abundances_data_1, elements_1, threshold)
    elements_2, abundances_data_2 = element_remover(abundances_data_2, elements_2, threshold)
    ...
    os.makedirs("Abundances_frames", exist_ok=True)
    frames = []
    ...
    for i in range(n_frames):
        ...
        filename = f"Abundances_frames/frame_{i:03d}.png"
        plt.savefig(filename, dpi=100)
        plt.close(fig)
        frames.append(filename)
    ...
    #Create animation
    clip = ImageSequenceClip(frames, fps=fps)
    clip.write_videofile("Abundances.mp4", codec="libx264")
...
def plot_abuns(ax, abundances, masses, ages, i, mass_lim, which_star, threshold, element_styles, style_cycle, fontsize, left):
    ...
    ax.text(0.5, 0.97, Label_Star_Age(ages, i),
    ...
    if left == True:
        ...
        ax.tick_params(right=True, left=False,
            axis='y', f='in', which='both', labelsize=fontsize)
...
def get_profile_and_ages(logs):
    try:
        profiles_index = MesaProfileIndex(file_name=f'{logs}/profiles.index')
        profiles = profiles_index.profile_numbers

        #Ages
        m = MesaLogDir(log_path=logs)
        h = MesaData(file_name=f'{logs}/history.data')
...
def get_conv_or_rad(index, logs):
...
def get_energy_gen_indexes(logs, profiles):

    energy_indexes = []

    for i in range(len(profiles)): #
        energy_gen =  get_energy_gen(logs, i) #Energy generation (T/F) for each zone in a profile
...
def get_conv_or_rad_indexes(logs, profiles):

    conv_or_rad_indexes = []
        
    # This loop works the same as get_energy_gen_indexes
    for i in range(len(profiles)):
        conv_or_rad = get_conv_or_rad(i, logs)
        indexes = []
        stored = "radiative"
...
def get_mass_indexes_by_energy_gen(logs, profiles):
        
    energy_gen_indexes = get_energy_gen_indexes(logs, profiles) #Get the list of lists based on which star
    energy_indexed_masses = [] #Empty list
    for i in range(len(profiles)):
        profile_indexes = energy_gen_indexes[i] #Pull the energy gen indexes for a given profile
        masses = get_mass(logs, i) #Pull the masses for a given profile
...
#Produces a list of lists.
def get_mass_indexes_by_conv_or_rad(logs, profiles):
    
    # This loop works the same as get_mass_indexes_by_energy_gen
    conv_or_rad_indexes = get_conv_or_rad_indexes(logs, profiles)
    conv_indexed_masses = []
    for i in range(len(profiles)):
        profile_indexes = conv_or_rad_indexes[i]
        masses = get_mass(logs, i)
        inner_list = []
        for index in profile_indexes:
...
#Actual plotting function
def plot_kip(logs, which_star, ax):
    ...
    conv_indexed_masses = get_mass_indexes_by_conv_or_rad(logs, profiles)
    energy_indexed_masses = get_mass_indexes_by_energy_gen(logs, profiles)
    ...
    custom_legend = [Patch(facecolor='dodgerblue',edgecolor='black', label='Convective', alpha=alpha),
    ...
    ax.legend(handles=custom_legend, loc="center left",
    ...
    for i in range(len(ages)):
        masses.append(np.max(get_mass(logs, i)))
...
def plot_Kippenhahn(logs1, logs2):
    ...
    ax1 = plt.subplot(gs[0, 0])
    ax2 = plt.subplot(gs[0, 1]) 
    plot_kip(logs1, "Donor", ax1)
    plot_kip(logs2, "Companion", ax2)

    plt.show()
    plt.savefig("Kippenhahn.png")

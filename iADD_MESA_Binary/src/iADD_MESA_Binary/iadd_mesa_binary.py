...
def get_Binary_Data(PM):

    if not PM:
        
        try: #Read in data, with error handling for file reading issues.
            h1 = MesaData('binary_logs1/history.data') #Primary or donor star
            h2 = MesaData('binary_logs2/history.data') #Secondary or companion star
            b = MesaData('binary_history.data') #Binary data
        except (OSError, IOError, FileNotFoundError) as e: #Handle file reading errors, such as missing files or incorrect paths.
            print(f"Error: Could not read the data files.")
            print(f"Details: {e}")
            return None
    ...
    try:
        if parameter in profiles_parameters:
            ...
            if which_star == '1' or which_star == '2':
                ...
                if profile_n_input == "One":
                    ...
                    which_profile = input()
                    profile = MesaData(f"binary_logs{which_star}/profile{which_profile}.data")
                    data = profile.data(parameter)
                elif profile_n_input == "All":
                    data = []
                    profiles_index = MesaProfileIndex(f'binary_logs{which_star}/profiles.index')
                    profiles = profiles_index.profile_numbers
                    for i in profiles:
                        profile = MesaData(f'binary_logs{which_star}/profile{i}.data')
        ...
        if parameter in history_parameters:
            ...
            if history_input == '1' or history_input == '2':
                h = MesaData(f"binary_logs{history_input}/history.data")
                data = h.data(parameter)
        
        if parameter in binary_history_parameters:
            h = MesaData("binary_history.data")
            data = h.data(parameter)
...
def plot_Mass_Transfer(fps, PM):
    ...
    if PM:
        Age, a, M1, M2, Total_M, R1, T1, Lum1, period = data
...
#For examples, see documentation.
#fps is the frames/s of the generated movie, ~12 generally provides good temporal resolution. Must be a positive integer.
def plot_Hertzsprung_Russel(fps, observers, PM):
...
def plot_Roche_Lobe(fps, PM):
    ...
    for i in range(n_frames):
        ...
        if PM:
            plot_RL(ax1, lpoints, lpot, p, R1[i], 0, a[i], Ages, ext_vect, q, colors1[i], 'white')
...
def plot_Abundances(fps, threshold, PM):
    ...
    try: #Read in data, with error handling for file reading issues
        M1 = mp.MESA()
        M1.loadHistory(f= 'binary_logs1')
        profiles_index_1 = MesaProfileIndex(file_name = 'binary_logs1/profiles.index')

        #For star ages
        m1 = MesaLogDir(log_path= 'binary_logs1')
        h1 = MesaData(file_name= 'binary_logs1/history.data')
    ...
    for j in profiles_1:
        profile = MesaData(f"binary_logs1/profile{j}.data")
    ...
    if not PM:
        ...
        try: #Read in data, with error handling for file reading issues
            M2 = mp.MESA()
            M2.loadHistory(f = 'binary_logs2')
            profiles_index_2 = MesaProfileIndex(file_name = 'binary_logs2/profiles.index')

            #Star 2 ages
            m2 = MesaLogDir(log_path = 'binary_logs2')
            h2 = MesaData(file_name = 'binary_logs2/history.data')
        except (OSError, IOError, FileNotFoundError) as e:
            print(f"Error: Could not read the data files. Check your paths.")
            print(f"Details: {e}")
            return None
        ...
        for k in profiles_2:
            profile = MesaData(f"binary_logs2/profile{k}.data")
...
#Actual plotting function
def plot_kip(logs, which_star, ax, PM):
    ...
    # 3. Handle legend placement
    if PM:
        # Single plot: Place legend to the right of ax1
        ax.legend(
        ...
        # Two plots: Place legend to the right of ax1 
        if which_star == "Donor": 
            ax.legend(
                handles=custom_legend, 
                loc="center left",
...
def plot_Kippenhahn(PM):
    ...
    if PM:
        ...
        gs = gridspec.GridSpec(1, 1, figure=fig)
        ax1 = plt.subplot(gs[0, 0])
        plot_kip('binary_logs1', "Donor", ax1, PM)
    ...
    if not PM:
        ...
        plot_kip('binary_logs1', "Donor", ax1, PM)
        plot_kip('binary_logs2', "Companion", ax2, PM)
